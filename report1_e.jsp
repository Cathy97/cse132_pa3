        ...
        <tr>
            ...
            <%
                    ...
                    if (action != null && action.equals("search")) {
                        ...
                        "SELECT C.NAME AS CONCENTRATION_NAME, C.COURSE_NUM AS COURSE_NUM, S.QUARTER AS QUARTER, S.YEAR AS YEAR FROM CONCENTRATION C JOIN ( SELECT DISTINCT CL.COURSE_NUM AS COURSE_NUM, CL.QUARTER AS QUARTER, CL.YEAR AS YEAR FROM CLASS CL, CONCENTRATION C WHERE CL.COURSE_NUM = C.COURSE_NUM AND ((CL.YEAR >= 2018 AND CL.QUARTER != 'Spring') OR (CL.YEAR>=2019))) S ON (C.COURSE_NUM = S.COURSE_NUM) WHERE C.DEPT_NAME = ? AND C.COURSE_NUM NOT IN ( SELECT S.COURSE_NUM FROM TAKEN T, SECTION S WHERE T.SSN = ? AND T.SECTION_ID = S.SECTION_ID AND T.GRADE IN ('A+','A','A-','B+','B','B-','C+','C','C-','D','S')) ORDER BY S.YEAR ASC, CASE WHEN S.QUARTER = 'Winter' THEN 1 WHEN S.QUARTER = 'Spring' THEN 2 WHEN S.QUARTER = 'Fall' THEN 3 END");
